...
Public Class ToolBoxFavoritos
  Private mSistemas As New SistemaList

  Public Sub AgregarSistema(ByVal _Sistema As Sistema, ByVal _Recargar As Boolean)
    ...
    If _existe AndAlso _Recargar Then
    ...
    If Not _existe Then
      ...
      If _Sistema.Probarconexion Then
        ...
        For Each _grupoopcion As GrupoOpcion In _grupoopciones
          ...
          For Each _opcion As Opcion In _grupoopcion.OpcionesFavoritos
  ...
  Public ReadOnly Property OpcionActual() As Opcion
    Get
      If Me.treeView1.SelectedNode Is Nothing Then
        Return Nothing
      ElseIf TypeOf Me.treeView1.SelectedNode.Tag Is Opcion Then
  ...
  Private Sub btnquitarfavoritos_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnquitarfavoritos.Click
    Dim _opcion As Opcion = OpcionActual
    If _opcion IsNot Nothing Then
      _opcion.Restriccion.Restri_Favorito = False
      _opcion.Restriccion.Guardar()

      AgregarSistema(SistemaActual, True)
    End If
  End Sub
